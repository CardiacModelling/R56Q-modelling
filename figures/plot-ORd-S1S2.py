...
# Run, using the same threshold as before
s.pre(bcl * 100)
d4, apds = s.run(bcl*n_APs, apd_threshold=vt)

colors = [(0.11765,0.23529,1.0), 'red', 'orange']
...
ax2.plot(d4['engine.time'], d4['ikr.IKr'], linewidth=1, c=colors[2])
ax2.set_ylabel('Current (A/F)', fontsize=9)
ax2.set_xlabel('Time (ms)', fontsize=9)
ax2.set_xlim([8000, 11000])
ax2.grid(True)

if args.show == True:
    plt.show()
else:
    filename = 'ORd-model-S1S2-ICaL-' + str(ICaL)
